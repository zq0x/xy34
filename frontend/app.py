...
def update_test_vllms():
    try:
        global test_vllms
        redis_connection(**test_call_update_all)
        test_vllms = get_test_vllms()
        # print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] [update_test_vllms] UPDATE! test_vllms: {test_vllms}')
        return f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] '
        # return test_vllms[0]["id"]
        # return update_test_vllms
        # return test_vllms
    except Exception as e:
        print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] [update_test_vllms] {e}')
        return f'err {str(e)}'
...
def create_app():
    with gr.Blocks() as app:
        ...
        **Note**: _[vLLM supported models list](https://docs.vllm.ai/en/latest/models/supported_models.html)_        
        ...
        # vllm_timer = gr.Timer(5,active=False)
        # vllm_timer.tick(
        #     update_test_vllms,
        #     None,
        #     outputs=[test_vllm_radio_out],
        #     show_progress=False
        # )
        
                
        # vllm_header_timer = gr.Timer(5,active=True)
        # vllm_header_timer.tick(
        #     update_test_vllms,
        #     test_vllm_radio_out
        # )
        ...
        # container_timer = gr.Timer(5,active=False)
